...
def _mention_excluded_food(message: str, excluded: Sequence[str]) -> str | None:
    ...
    2026-08-23 独立验证修复：支持**单字食材**（虾/蟹/鱼/蛋）——「虾能吃吗」
...
# 刁难场景修复：剔除标记后紧跟的食物形态单字（碎/酱/粉/片…）是残字，一并吞掉。
...
# 刁难场景：极端危险行为不得当普通问答/FAQ 处理，必须风险预警 + 就医引导 + 免责。
# 第七波护栏：子串触发词必须加语义约束——
...
_DANGER_REPLIES: dict[str, str] = {
    "extreme_fast": (
        "⚠️ 连续断食/极低热量饮食可能导致低血糖、头晕、脱水甚至晕厥，"
...
def _jin_scale(message: str) -> float | None:
    ...
    终极压测（刁难场景）：「一斤去皮生鸡胸肉…多少大卡」须按 500g 折算，
