...
| `tests/` | pytest 全量测试 + 红线回归（R1-R8）+ P2/P3/P5 迭代用例 | 环境隔离，不污染线上数据；当前 220 用例全绿 |
...
自动化红线回归：`python -m pytest tests -q`（250 passed）。含 100 条攻击式指令压力测试（暴风雪）、外部强模型审查与 50 条用户视角刁难压测，缺陷闭环并回归锁死（暴风雪 8 + 外部审查 6 + 三审 7 + 终极压测 13 + 第七波回归修复 17），详见 `30_测试记录/2026-08-22_暴风雪压力测试_摘要版.md`。
